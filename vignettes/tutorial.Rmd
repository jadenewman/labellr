...
of rules that are defined in advance (i.e. is deterministic). Writing code that performs this categorisation may
be lengthy, making it difficult to debug and understand. For example, you may have a medical
...
As an example, suppose a medical clinic performs either X-ray, FMRI or minor surgery. Using 
your preferred tabular data editor you outline the rules that define each level of the summary variable 'treatment'. (Alternatively these can be built in R using tbd)

A first step is to indicate the datatype for each variable that will be used in the data classification. `labellr` uses three datatypes, `date`, `num` and `char` - which should indicated in the `datatype` row of the definitions table. The correspondance of `date`, `num` and `char` to R classes is shown in table 1. 

```{r}
t1 <- data.frame(c("num","char","date"),
                 c("numeric","character","..."))
knitr::kable(t1,caption = "Table 1. labellr datatype and R class correspondance",
             col.names = c("labellr datatype","R class"))
```

There exist different rulesets depending on the datatype is chosen (below is from `?labellr::classify`):

_Rulesets for variable type `num`_:  
...
_Rulesets for variable type `char`_:  
